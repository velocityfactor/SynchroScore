...
public class SwimmerManager {
    ...
    public static List<Swimmer> getSwimmers(Season season) {
        Query swimmerQuery = ScoreApp.getEntityManager().createNamedQuery("Swimmer.findBySeasonOrderByTeamAndName");
        swimmerQuery.setParameter("season", season);
        List<Swimmer> swimmers = swimmerQuery.getResultList();
        return swimmers;
    }
    ...
    public static Integer getNextLeagueNumber() {
        Query swimmerQuery = ScoreApp.getEntityManager().createNamedQuery("Swimmer.findMaxLeagueIdBySeason");
        swimmerQuery.setParameter("season", ScoreApp.getCurrentSeason());
        return (Integer) swimmerQuery.getSingleResult() + 1;
    }

    public static void exportSwimmers(File file, Season season) throws IOException {
        FileWriter writer = new FileWriter(file);
        writer.write("League#,Last name,First name,Level,Team\n");
        for (Swimmer swimmer : getSwimmers(season)) {
            String s = swimmer.getLeagueNum() + ",\""
                    + swimmer.getLastName() + "\",\""
    ...
    public static void importSwimmers(File file, Season season) throws IOException {
        ...
        if (header.length < 5 || !"League#".equals(header[0])
                ...
                || !"Level".equals(header[3])
                || !"Team".equals(header[4])) {
        ...
        for (String[] row : rows) {
            ...
            try {
                ...
                sb.append("Invalid league number on line ").append(ln).append("\n");
            }
        ...
        if (sb.length() == 0) {
            ...
            try {
                for (Swimmer swimmer : swimmers) {
                    ScoreApp.getEntityManager().persist(swimmer);
                }
