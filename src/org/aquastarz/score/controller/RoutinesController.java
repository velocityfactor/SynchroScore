...
public class RoutinesController {
		...
		try {
			JasperReport jasperReport = (JasperReport) JRLoader
					.loadObject(getClass().getResourceAsStream(
							"/org/aquastarz/score/report/RoutinesOrder.jasper"));
	...
	public static void showRoutinesLabelsReport(Meet meet, int skipLabels,
			boolean showNovice, boolean showIntermediate) {
		ScoreController.calculateMeetResultsIfNeeded(meet);
		try {
			JasperReport jasperReport = (JasperReport) JRLoader
					.loadObject(RoutinesController.class
							.getResourceAsStream("/org/aquastarz/score/report/RoutineLabels.jasper"));
			List<Routine> routines = new LinkedList<Routine>();
			for (int i = 0; i < skipLabels; i++) {
				routines.add(null);
			}
			routines.addAll(ScoreController.generateRoutineLabels(meet,
					showNovice, showIntermediate));
